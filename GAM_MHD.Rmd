...
#WHen comparing all 3 models, gam_modIY is the best model with AIC 12813.5. 
#gam_mod = 12317.86
#gam_modSS = 12859.28
AIC(gam_modIY, gam_modSS)
...
Section 9 
```{r}
vis.gam(gam_modIY,  plot.type = "contour", ylab = "Day_Year", xlab = "Year.Sampled", color = "terrain", main = "Interaction between day of year and season", theme(legend.position = "top"))

vis.gam(x= gam_modIY,plot.type = "contour",color="terrain",theta=-35)  
...
STyes = gam(Infection~ s(Longitude_city,Latitude_city) + s(Year.Sampled, k = 8)+ ti(Longitude_city, Latitude_city, Year.Sampled, d=c(2,1)) + s(Volunteer, bs="re"),
                          data=data,family=binomial,method="REML")
...
predict_infection = expand.grid(
  Latitude_city= seq(min(data$Latitude_city), 
                max(data$Latitude_city),
                length=50),
  Longitude_city = seq(min(data$Longitude_city),
                  max(data$Longitude_city),
                  length=50),
  Volunteer = data$Volunteer, 
  Year.Sampled = seq(1,8,by=1)
)
```
...
Section 12 
```{r}
##(DONT DO THIS! It is too computationally intensive without the cluster) 


predict_infection$model_fit = predict(STyes,
                                    predict_infection,type = "response")

save(model_fit, file="model_fit.Rdata")

```

Section 13 
```{r}
ggplot(aes(Longitude_city, Latitude_city, fill= model_fit),
