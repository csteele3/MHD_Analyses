...
```{r Merge freeze and infection data}
...
Calculate the number of freeze days in 30 day windows. 
```{r}
...
#Add date column back into
Freeze30 <- cbind(date = DFS$date,DFSroll )

#Turn station ID back into a single column for model analysis   
#Freeze7<-Freeze7 %>%  gather(id, freeze.day,c(2:ncol(Freeze7))) 
Freeze30<-Freeze30 %>% pivot_longer(cols=2:ncol(Freeze30),names_to="id",values_to="freeze.day")

#Check to ensure that there is a range of freezes 
Freeze30 %>% group_by(freeze.day) %>% tally()

#Merge 
MHD_freeze30<-left_join(MHD, Freeze30, by=c("date", "id"))
summary(MHD_freeze30)
