...
#caz's code - not working correctly check number of rows in tmp
tmp<-Weather_Hardi %>% 
  tidyr::pivot_wider(id_cols=c(id),names_from=c(Year,Month,Day),values_from=freeze.day) %>% 
  tidyr::pivot_longer(cols=2:ncol(tmp),names_to="date",values_to="freeze.day") %>%
  tidyr::pivot_wider(id_cols=date,names_from=id,values_from=freeze.day)

library(zoo)
tmp2<-as.data.frame(tmp[,2:ncol(tmp)] %>% zoo::rollapply(width=7,FUN=function(x){sum(as.numeric(x),na.rm=T)},by.column=TRUE))
...
############################################################
#Create the 7 DAY SUMMED NUMBER OF FREEZES Column#
#############################################################
Weather_Hardi_SUMS = Weather_Hardi_SUMS %>%
  dplyr::group_by(id, Year) %>%
  dplyr::arrange(id, Year, Day) %>%
  dplyr::mutate(freeze.lag0 = lag(freeze.day, n = 1)) %>%
  dplyr::mutate(freeze.7.previous = zoo::rollapply(data = freeze.lag0, 
                                     width = 7, 
                                     FUN = sum, 
                                     align = "right", 
                                     fill = NA, 
...
#Check to ensure that there is a range of freezes 
Weather_Hardi_SUMS %>% group_by(freeze.7.previous) %>% tally()
```
...
Weather_Hardi_SUMS$id<-as.factor(Weather_Hardi_SUMS$id)

#Merge 
data<-merge(MHD, Weather_Hardi_SUMS, by=c("date", "id"))
```
...
F7_gam <-mgcv:: gam(Infection ~ s(freeze.7.previous, k=6) + s(Year.x, k= 5) , family = binomial, link = logit, data = data)
summary(F7_gam)
AIC(F7_gam)
#18864.28
