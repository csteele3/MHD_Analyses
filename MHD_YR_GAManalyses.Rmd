...
STdayear = gam(Infection~ s(longitude,latitude) + s(Day_Year)+ ti(longitude, latitude, Day_Year, d=c(2,1)) + s(Volunteer, bs="re"), data=data,family=binomial,method="REML")

save(STdayear, file="STdayear.Rdata")

summary(STdayear)
plot(STdayear,scheme=2)
...
predict_infection = expand.grid(
  ...
  longitude = seq(min(data$longitude),
                  ...
                  length=100),
  Volunteer = levels(data$Volunteer)[69],  
  Day_Year = seq(min(data$Day_Year),
                  max(data$Day_Year),
                  length=365))
...
predict_infection2$model_fit = predict(STdayear,
...
for(g in 1:length(groups))
{
  temp<-predict_infection2[with(predict_infection2 %>% dplyr::select("longitude","latitude"), 
                               inSide(se_map %>% filter(group==groups[g]) %>% dplyr::select("longitude","latitude"),longitude,latitude)),]
  predict_infection_clipped2<-rbind(predict_infection_clipped2,temp)
...
```{r}
gg<-ggplot(aes(longitude, latitude,  fill= model_fit), 
       data=predict_infection_clipped2)+ #%>% filter(Year.Sampled==2011))+ 
  geom_tile()+
  #facet_wrap(~Year,nrow=4)+
  scale_fill_viridis(option="magma", "Infection Prevalence")+
  geom_polygon(data=se_map,aes(x=long,y=lat,group=group),color="black",inherit.aes = FALSE,alpha=0)+
