...
gam_modTIME <- gam(Infection ~  s(Day_Year, bs = "cc") + s(Year,  k = 8) + ti(Day_Year, bs = "cc", by = Year, k= 8) + s(Volunteer, bs="re"), family = binomial, method = "REML", data = data)
...
STmonth = gam(Infection~ s(longitude,latitude) + s(Month) + ti(longitude, latitude, Month, d=c(2,1))  + s(Volunteer, bs="re"), data=data,family=binomial,method="REML")
...
save(STmonth, file="STdayear.Rdata")
...
predict_infection2$model_fit = predict(STmonth,
...
predict_infection_clipped$Month<-as.character(predict_infection_clipped$Month)
predict_infection_clipped$Month <- revalue(x = predict_infection_clipped$Month, 
c("1" = "Jan", "2" = "Feb", "3" = "March", "4" = "April", "5" = "May", "6" = "June", "7" = "July", "8" = "Aug",  "9" = "Sept", "10" = "Oct", "11" = "Nov", "12" = "Dec" ))


neworder<- c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec")
library(plyr)  ## or dplyr (transform -> mutate)
predict_infection_clipped$Month <- arrange(transform(predict_infection_clipped,
             Month=factor(Month,levels=neworder)),Month)

predict_infection_clipped$Month  <- factor(predict_infection_clipped$Month , levels = c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec"))
