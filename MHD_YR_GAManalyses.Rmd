...
#Merge 
PRCPData30<-left_join(MHD, PRCPData, by=c("date", "id"))
summary(TMINData7)
```
...
###################PRCP + TMAX Raw GAM###########################################
MHD$Volunteer<-as.factor(MHD$Volunteer)

TP_gam <-mgcv:: gam(Infection ~ s(PRCP) + s(TMAX2) + s(Year, k= 5) + s(longitude,latitude) + s(Volunteer,bs="re") , family = binomial, link = logit, data = MHD)
summary(TP_gam)
AIC(TP_gam)
#
...
PXT_7day<-left_join(PRCPData7, TMAXData7, by=c("Day", "Year", "id"))

###################PRCP + TMIN 7 Day Average GAM###########################################
PXT_7day$Volunteer.x<-as.factor(PXT_7day$Volunteer.x)

TPa_gam <-mgcv:: gam(Infection.x ~ s(PRCP.7day) + s(TMAX.7day) + s(Year, k= 5) + s(longitude.x,latitude.x) + s(Volunteer.x,bs="re") , family = binomial, link = logit, data = PXT_7day)
summary(TPa_gam)
AIC(TPa_gam)
#
...
TXP_gam <-mgcv:: gam(Infection ~ s(PRCP) + s(TMAX2) + ti(PRCP, TMAX2)+ s(Year, k= 5) + s(longitude,latitude) + s(Volunteer,bs="re") , family = binomial, link = logit, data = MHD)
summary(TP_gam)
AIC(TP_gam)
#
plot(TP_gam, pages = 1,  trans = plogis,  shift = coef(TP_gam)[1], seWithMean = TRUE, rug = FALSE, shade = TRUE, shade.col = "lightgrey")

vis.gam(TP_gam, view=c("PRCP","TMIN2"),plot.type="contour", color = "terrain", contour.col = "black") 
```
...
TXPa_gam <-mgcv:: gam(Infection.x ~ s(PRCP.7day) + s(TMAX.7day) + ti(PRCP.7day, TMAX.7day)+ s(Year, k= 5) + s(longitude.x,latitude.x) + s(Volunteer.x,bs="re") , family = binomial, link = logit, data = PXT_7day)
summary(TPa_gam)
AIC(TPa_gam)
#
plot(TPa_gam, pages = 1,  trans = plogis,  shift = coef(TPa_gam)[1], seWithMean = TRUE, rug = FALSE, shade = TRUE, shade.col = "lightgrey")

vis.gam(TMAXP_gam, view=c("PRCP","TMAX2"),plot.type="contour", color = "terrain", contour.col = "black") 
...
TMAXfiltered<- filter(MHD, TMAX > "30")
