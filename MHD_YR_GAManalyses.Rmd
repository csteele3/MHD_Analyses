...
library(zoo)
DFSroll<-rollapply(data = DFS2 [,c(2:96)],width = 7, 
                                     FUN = sum, 
                                     align = "right", 
                                     fill = NA, 
                                     by.column = TRUE,
...
Freeze7 <- cbind(DFSroll, date = DFS2$date)
#Is it risky to assume these are all correctly aligned with the correct date? 

#Turn station ID back into a single column for model analysis   
Freeze7<-Freeze7 %>%  gather(id, freeze.day,c(1:95)) 
...
```{r Merge freeze and infection data: PROBLEM HERE?}
...
#Merge 
data<-left_join(MHD, Freeze7, by=c("date", "id"))
#Lost 3,000 observations for some reason 

#It appears as though there are only 88 stations in the MHD data frame. Perhaps some are not pairing correctly with the weather stations. 
Fids <- data.frame( "Fid" = unique(Freeze7$id)) 
                   
Mids <- data.frame("Mid"=unique(MHD$id))
...
F7_gam <-mgcv:: gam(Infection ~ s(freeze.day, k=6) + s(Year, k= 5) + s(longitude,latitude) , family = binomial, link = logit, data = data)
summary(F7_gam)
AIC(F7_gam)
#18864.28
...
##############For comparison TMIN model 
TMIN_gam <-mgcv:: gam(Infection ~ s(TMIN) + s(Year, k= 5) + s(longitude,latitude), family = binomial, link = logit, data = data)
summary(TMIN_gam)
AIC(TMIN_gam)
#18864.28
plot(TMIN_gam, pages = 1,  trans = plogis,
     shift = coef(F7_gam)[1], seWithMean = TRUE, rug = FALSE, shade = TRUE, shade.col = "lightgrey")
```
...
```{r}
#############################################################
#Create the 30 DAY SUMMED NUMBER OF FREEZES Column#
#############################################################
Weather_Hardi_SUMS = Weather_Hardi %>%
  dplyr::group_by(id, Year) %>%
  dplyr::arrange(id, Year, Day) %>%
  dplyr::mutate(freeze.lag1 = lag(freeze.day, n = 1)) %>%
  dplyr::mutate(freeze.30.previous = zoo::rollapply(data = freeze.lag1, 
                                     width = 30, 
                                     FUN = sum, 
                                     align = "right", 
                                     fill = NA, 
...
#Check to ensure that there is a range of freezes 
Weather_Hardi_SUMS %>% group_by(freeze.30.previous) %>% tally()



Weather_Hardi<-Weather_Hardi  %>% dplyr::arrange(id,Year,Month,Day)
...
#############################################################)
#Create the 60 DAY SUMMED NUMBER OF FREEZES Column#
#############################################################
Weather_Hardi_SUMS = Weather_Hardi_SUMS %>%
  dplyr::group_by(id, Year) %>%
  dplyr::arrange(id, Year, Day) %>%
  dplyr::mutate(freeze.lag2 = lag(freeze.day, n = 1)) %>%
  dplyr::mutate(freeze.60.previous = zoo::rollapply(data = freeze.lag2, 
                                     width = 60, 
                                     FUN = sum, 
                                     align = "right", 
                                     fill = NA, 
...
#Check to ensure that there is a range of freezes 
Weather_Hardi_SUMS %>% group_by(freeze.60.previous) %>% tally()
