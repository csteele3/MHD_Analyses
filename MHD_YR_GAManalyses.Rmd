...
PRCP 7 Day Window Calculations
```{r TMIN 7 Day Window Calculations}
library(tidyr)
TMINSubset = subset(Weather_Hardi, select = c(id, date, TMIN) )
TMINS<-TMINSubset %>% pivot_wider(id_cols=date,names_from = id,values_from=TMIN) %>% arrange(date)
...
newdat<-aggregate(x=TMINData7$Infection,
          by=list(TMINData7$Sex,TMINData7$Year),
          FUN=mean)

p <- ggplot() +geom_point(data = newdat, aes(x=Group.2, y=x, group=Group.1, colour = Group.1)) + stat_summary(fun="mean")
print(p)
```
...
TMAX Previous 7 Day Average GAM
```{r TMAX 7 Day GAM}
###################TMIN 7 Day Average##################################
...
P_gam <-mgcv:: gam(Infection ~ s(PRCP) + s(Year, k= 5) + s(longitude,latitude) + s(Volunteer,bs="re") , family = binomial, link = logit, data = TMINData7)
summary(P_gam)
AIC(P_gam)
#12372.3
...
```{r PRCP X TMIN Raw GAM}
library(mgcv)

###################TMIN RAW###########################################
PRCP$Volunteer<-as.factor(TMINData7$Volunteer)
P_gam <-mgcv:: gam(Infection ~ ti(PRCP, TMIN2) + s(Year, k= 5) + s(longitude,latitude) + s(Volunteer,bs="re") , family = binomial, link = logit, data = TMINData7)
summary(P_gam)
AIC(P_gam)
#12372.3
plot(P_gam, pages = 1,  trans = plogis,  shift = coef(P_gam)[1], seWithMean = TRUE, rug = FALSE, shade = TRUE, shade.col = "lightgrey")
    
vis.gam(P_gam, view=c("PRCP","TMIN2"),plot.type="contour", color = "terrain", contour.col = "black") 

P_gam <-mgcv:: gam(Infection ~ s(PRCP) + s(TMIN2) + ti(PRCP, TMIN2) + s(Year, k= 5) + s(longitude,latitude) + s(Volunteer,bs="re") , family = binomial, link = logit, data = TMINData7)
